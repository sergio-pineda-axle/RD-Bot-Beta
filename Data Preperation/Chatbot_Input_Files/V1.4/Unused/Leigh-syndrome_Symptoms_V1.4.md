# Symptoms for [Leigh Syndrome](https://rarediseases.info.nih.gov/diseases/6877/leigh-syndrome)]


## [Affected Body Systems and Corresponding Defintions]
Symptoms related to Leigh syndrome may affect different systems of the body. 
Note for Chatbot: these body systems correspond to the "Body Systems Category(-ies)" values referenced within each symptom
...
## [List of Leigh syndrome Symptoms]
Note for Chatbot guidance in navigating these symptoms:

	The following list provides the symptom(s) associated with leigh syndrome. A description of symptom, synonyms of the symptom, which body system(s) are affected by the symptom and the symptom's frequency among confirmed cases of Leigh syndrome are provided. This is not an exhaustive list - not all possible symptoms may be listed. 

    If the end user asks about a disease's symptoms, provide those with  higher "Frequency" values first and limit your list to 10, unless requested to provide more.
	
    If the end user asks about a specific body system being affected refer to the values of "Body System Category(-ies)" to match up, providing those with higher "Frequency" values first and limit your list to 10, unless requested to provide more

	If the end user asks about common versus uncommon symptoms, refer to Frequency value.
